...
class MakeYaml
  ...
  def make_uuids(indent, prefix, count)
     ...
     (1..count.to_i).each do |i|
        puts "#{"  " * indent}#{prefix}#{i}: '#{SecureRandom.uuid}'"
     end
  end
  ...
  def make_serverlist(indent, key, count)
    ...
    puts "#{"  " * indent}#{key}:"
    (1..count.to_i).each do |i|
      puts "#{"  " * indent}  - 'foo#{i}.mycompany.org'"
    end
  end
  ...
  def make_ips(indent, prefix, count)
    ...
    (1..count.to_i).each do |i|
      puts "#{"  " * indent}#{prefix}#{i}: '127.0.0.#{i}'"
    end
  end
  ...
  def make_nested_map_fields
    ...
    (1..num_fields).each do |i|
      ...
      case field_type
      when "uuid"
        make_uuids(2, "uuid#{i}_", 1)
      when "serverlist"
        make_serverlist(2, "servers#{i}", 2 + rand(10))
      when "iplist"
        make_iplist(2, "ips#{i}", 2 + rand(10))
      when "ip"
        make_ips(2, "ip#{i}_", 1)
      end
    end
